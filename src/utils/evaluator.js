...
export const calculateFishingScore = (data, point = {}) => {
  ...
  const microVar = ((seed % 20) - 10) / 10; // -1.0 ~ +1.0

  // [2] 베이스 점수
  let score = 60 + (seed % 15) + microVar;

  // [3] 풍속 보정
  const wind = parseFloat(data.wind?.speed || 2.1) + microVar;
  if      (wind > 14) score -= 60;
  else if (wind > 10) score -= 35;
  else if (wind >  8) score -= 22;
  else if (wind >  5) score -= 8;
  else if (wind <  3) score += 10;
  else if (wind <  4) score += 6;

  // [4] 파고 보정
  const wave = parseFloat(data.wave?.coastal || 0.4) + (microVar / 5);
  if      (wave > 2.5) score -= 55;
  else if (wave > 2.0) score -= 40;
  else if (wave > 1.2) score -= 22;
  else if (wave > 0.8) score -= 8;
  else if (wave < 0.3) score += 7;
  else if (wave < 0.5) score += 3;

  // [5] 수온 보정
  const sst = parseFloat(data.sst || 15) + (microVar / 4);
  if (sst < 8 || sst > 30)          score -= 35;
  else if (sst < 11 || sst > 27)    score -= 20;
  else if (sst >= 17 && sst <= 22)  score += 12;
  else if (sst >= 14 && sst <= 25)  score += 5;
