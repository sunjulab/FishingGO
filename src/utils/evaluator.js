...
const FISH_ALERTS = {
  ...
  '무늬오징어':{ alert: '에깅에 좋은 수온입니다. 파고 확인 후 저크&폴 액션으로 공략하세요.',       gear: 'Eg 2.5~3.5호, 저크&폴 액션' },
  ...
  // [v2.4] 신규 2종
  '열기':      { alert: '열기(빨간 열어)는 겨울~봄 동해 선상낚시의 꽃입니다. 깊은 수심 외줄낚시로 공략.', gear: '외줄낚시 바늘 5~8호, 상깃/오징어 미끼' },
  '붕장어':    { alert: '붕장어(연안 맞좌)는 서해 여름~가을 야간 낚시의 화려입니다. 야간이 피크.', gear: '양바시 2~3호 링서위, 멍게/잡어 미끼' },
};
...
const SEASONAL_FISH = {
  '동해': {
    // ❄️ 겨울 — 가자미·이면수·볼락·열기 주력 (수온 최저기)
    1:  ['가자미', '이면수', '볼락', '열기'],    // 열기 선상낚시 1~5월 피크
    2:  ['가자미', '도다리', '볼락', '열기'],    // 볼락 겨울 최토활성
    // 봄 — 도다리·학꽁치 시즌 개막
    3:  ['도다리', '학꽁치', '볼락', '노래미'],  // 노래미 봄·가을 연중 동해 대표
    4:  ['도다리', '학꽁치', '감성돔', '노래미'],
    5:  ['감성돔', '학꽁치', '벵에돔', '농어'],  // 벵에돔+농어 5월 동해 연안 진입
    // ☀️ 여름 — 삼치 금어기(6~7월)
    6:  ['가자미', '볼락', '농어', '노래미'],    // 삼치 금어기
    7:  ['오징어', '전갱이', '볼락', '농어'],    // 삼치 금어기
    8:  ['오징어', '삼치', '고등어', '전갱이'],  // 8/1~ 삼치 해금 | 고등어 사비키 피크
    // 가을 — 방어·무늬오징어 피크
    9:  ['방어', '삼치', '무늬오징어', '고등어'],
    10: ['방어', '무늬오징어', '감성돔', '삼치'],
    // 늦가을~겨울 — 볼락 시즌 시작
  ...
  '서해': {
    ...
    // 봄 — 도다리·광어·참돔 시즌
    3:  ['도다리', '광어', '우럭', '숭어'],
    4:  ['광어', '우럭', '농어', '도다리'],    // 참돔 4월은 아직 이르른편 (5월말~ 본격) | 농어·광어 주력
    5:  ['광어', '우럭', '참돔', '갑오징어'], // 쭈꾸미 5/11~ 금어기 | 참돔 본격 시즌
    // ☀️ 여름 — 쭈꾸미·삼치 금어기
    6:  ['광어', '우럭', '참돔', '갑오징어'],  // 쭈꾸미·삼치 금어기
    7:  ['광어', '농어', '우럭', '갑오징어'],  // 쭈꾸미·삼치 금어기
    8:  ['농어', '광어', '고등어', '붕장어'],  // 쭈꾸미 금어기 | 붕장어 야간 피크
    // 가을 — 쭈꾸미 9/1~ 해금! 전어·고등어 피크
    9:  ['쭈꾸미', '갑오징어', '전어', '고등어'],
    10: ['쭈꾸미', '광어', '전어', '우럭'],
    // 늦가을~겨울
    11: ['우럭', '광어', '감성돔', '숭어'],
  ...
  '남해': {
    // ❄️ 겨울 — 감성돔·볼락·학꽁치 주력
    1:  ['감성돔', '볼락', '학꽁치', '도다리'],  // 학꽁치 겨울~봄 남해 대표
    2:  ['감성돔', '볼락', '학꽁치', '우럭'],
    // 봄
    3:  ['도다리', '감성돔', '볼락', '학꽁치'],  // 학꽁치 3월까지 이어짐
    4:  ['감성돔', '참돔', '벵에돔', '무늬오징어'],  // 무늬오징어 봄 대물시즌(4~6월)
    5:  ['참돔', '감성돔', '돌돔', '무늬오징어'],   // 돌돔 5월 시즌 시작
    // ☀️ 여름 — 한치·참돔·돌돔·농어 피크
    6:  ['참돔', '한치', '돌돔', '농어'],    // 돌돔 6~9월 피크 | 삼치 금어기
    7:  ['한치', '고등어', '돌돔', '농어'],  // 한치 최성기 | 고등어 7~10월 | 갈치·삼치 금어기
    8:  ['갈치', '부시리', '고등어', '돌돔'], // 8/1~ 갈치·삼치 해금 | 고등어 피크
    // 가을 — 갈치·방어·무늬오징어·고등어 피크
    9:  ['갈치', '방어', '무늬오징어', '고등어'],
    10: ['방어', '감성돔', '무늬오징어', '고등어'],  // 고등어 10월 피크
    // 늦가을~겨울
    11: ['감성돔', '방어', '무늬오징어', '볼락'],
    12: ['감성돔', '볼락', '학꽁치', '참돔'],  // 학꽁치 다시 등장
  },
  '제주': {
    // ❄️ 겨울 — 벵에돔 피크, 돌돔 연중 제주 대표
    1:  ['벵에돔', '감성돔', '돌돔', '볼락'],
    2:  ['벵에돔', '감성돔', '돌돔', '참돔'],  // 돌돔 제주 연중
    // 봄 — 돌돔 활성
    3:  ['벵에돔', '감성돔', '돌돔', '참돔'],
    4:  ['벵에돔', '참돔', '돌돔', '무늬오징어'],
    5:  ['자리돔', '무늬오징어', '돌돔', '참돔'],  // 돌돔 5월 피크
    // ☀️ 여름 — 한치 최성기! 돌돔 6~9월 피크
    6:  ['한치', '벵에돔', '돌돔', '농어'],    // 한치⭐⭐⭐ 돌돔 6~9월 피크
    7:  ['한치', '돌돔', '부시리', '방어'],    // 한치+돌돔 7월 최성기
    8:  ['한치', '방어', '돌돔', '부시리'],    // 돌돔 8월 피크
    // 가을 — 방어·삼치 피크. 돌돔 마무리
    9:  ['방어', '삼치', '돌돔', '무늬오징어'],  // 돌돔 9월 마무리
    10: ['방어', '벵에돔', '무늬오징어', '삼치'],
    // 늦가을~겨울
    11: ['벵에돔', '방어', '감성돔', '무늬오징어'],
    12: ['벵에돔', '감성돔', '돌돔', '볼락'],
