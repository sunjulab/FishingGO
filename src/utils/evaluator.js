...
const SEASONAL_FISH = {
  '동해': {
    // ❄️ 겨울 — 가자미·이면수·도다리·우럭 주력
    1:  ['가자미', '볼락', '방어', '우럭'],    // 볼락 1~3월 피크 | 방어 대방어 겨울시즌
    2:  ['가자미', '볼락', '방어', '도다리'],
    // 봄 — 도다리·학꽁치 시즌 개막, 감성돔 활성
    3:  ['도다리', '학꽁치', '볼락', '가자미'],  // 볼락 산란기 피크(1~3월)
    4:  ['도다리', '학꽁치', '감성돔', '벵에돔'],
    5:  ['감성돔', '학꽁치', '벵에돔', '무늬오징어'],
    // ☀️ 여름 — 삼치 금어기(6~7월). 오징어·볼락·농어 주력
    6:  ['가자미', '볼락', '무늬오징어', '농어'], // 삼치 금어기 | 무늬오징어 6월 산란 대물시즌
    7:  ['오징어', '무늬오징어', '전갱이', '볼락'], // 삼치 금어기 | 무늬오징어 대물
    8:  ['오징어', '무늬오징어', '삼치', '고등어'], // 8/1~ 삼치 해금 | 무늬오징어 가을 시즌 시작
    // 가을 — 방어·무늬오징어 피크
    9:  ['방어', '삼치', '무늬오징어', '고등어'],  // 고등어 마란 | 방어 최성기
    10: ['방어', '무늬오징어', '감성돔', '삼치'],
    // 회가울~겨울
    11: ['감성돔', '우럭', '방어', '무늬오징어'],
    12: ['가자미', '방어', '볼락', '이면수'],    // 방어 대방어 피크 | 볼락 확대
  },
  '서해': {
    // ❄️ 겨울 — 우럭·광어 주력
    1:  ['우럭', '광어', '숭어', '도다리'],
    ...
    // 봄 — 도다리·광어 시즌
    3:  ['도다리', '광어', '우럭', '숭어'],
    4:  ['광어', '우럭', '도다리', '농어'],    // 쭈꾸미 4월 가능 but 5/11~ 금어기
    5:  ['광어', '우럭', '농어', '갑오징어'],  // 쭈꾸미 5/11~ 금어기
    // ☀️ 여름 — 쭈꾸미·삼치 금어기. 광어·우럭·참돔 실조황⭐⭐⭐
    6:  ['광어', '우럭', '참돔', '갑오징어'],  // 쭈꾸미·삼치 금어기
    7:  ['광어', '농어', '백조기', '갑오징어'],  // 쭈꾸미·삼치 금어기 / 갑오징어 사이즈 궁음
    8:  ['농어', '광어', '백조기', '고등어'],    // 쭈꾸미 금어기 | 백조기·고등어 피크
    // 가을 — 쭈꾸미 9/1~ 해금! 전어·고등어 피크
    9:  ['쭈꾸미', '갑오징어', '전어', '고등어'],  // 전어·고등어 가을 피크
    10: ['쭈꾸미', '광어', '전어', '우럭'],      // 전어 시즌 안정기
    // 늦가을~겨울
    11: ['우럭', '광어', '감성돔', '숭어'],
    12: ['우럭', '광어', '숭어', '도다리'],
  },
  '남해': {
    // ❄️ 겨울 — 감성돔·볼락 주력
    1:  ['감성돔', '볼락', '벵에돔', '우럭'],   // 남해 겨울 벵에돔 종종 낙임
    2:  ['감성돔', '볼락', '벵에돔', '도다리'],
    // 봄
    3:  ['도다리', '감성돔', '볼락', '전갱이'],
    4:  ['감성돔', '참돔', '벵에돔', '무늬오징어'],   // 남해 무늬오징어 봄 대물시즌(4~6월)
    5:  ['참돔', '감성돔', '벵에돔', '무늬오징어'],   // 거제·통영·여수 에깅 대물시즌
    // ☀️ 여름 — 삼치·갈치 금어기. 한치·참돔·벵에돔·농어 피크
    6:  ['참돔', '한치', '농어', '벵에돔'],   // 삼치 금어기 | 농어 6월 본격시즌
    7:  ['한치', '전갱이', '농어', '참돔'],   // 한치 최성기 | 갈치·삼치 금어기
    8:  ['갈치', '부시리', '삼치', '농어'],   // 8/1~ 갈치·삼치 해금
    // 가을 — 갈치·방어 피크 | 무늬오징어 가을 마릿수 시즌
    9:  ['갈치', '방어', '무늬오징어', '부시리'],  // 남해 무늬오징어 가을 피크(9~11월)
    10: ['방어', '감성돔', '무늬오징어', '갈치'],   // 무늬오징어 10월 최성기
    // 회가울~겨울
    11: ['감성돔', '방어', '무늬오징어', '볼락'],   // 무늬오징어 11월까지 이어짐
    12: ['감성돔', '볼락', '우럭', '참돔'],
  },
  '제주': {
    // ❄️ 겨울 — 벵에돔 피크, 감성돔·볼락·돌돔 연중
    1:  ['벵에돔', '감성돔', '볼락', '돌돔'],
    2:  ['벵에돔', '감성돔', '볼락', '참돔'],
    // 봄
    3:  ['벵에돔', '감성돔', '자리돔', '참돔'],
    4:  ['벵에돔', '참돔', '자리돔', '무늬오징어'],
    5:  ['자리돔', '무늬오징어', '벵에돔', '참돔'],
    // ☀️ 여름 — 한치 최성기! 벵에돔·농어·자리돔 활발
    6:  ['한치', '벵에돔', '농어', '자리돔'],  // 실조황: 한치⭐⭐⭐ 벵에돔⭐⭐⭐ 농어⭐⭐⭐
    7:  ['한치', '자리돔', '부시리', '방어'],  // 한치 최성기
    8:  ['한치', '방어', '부시리', '무늬오징어'],
    // 가을 — 방어·삼치 피크. 벵에돔 회복
    9:  ['방어', '삼치', '무늬오징어', '한치'],
    10: ['방어', '벵에돔', '무늬오징어', '삼치'],
