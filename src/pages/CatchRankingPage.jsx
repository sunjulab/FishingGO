...
/* ─── 풀스크린 뷰어 ─────────────────────────────────────────── */
function PhotoViewer({ records, initialIndex, onClose, onLike, onDelete, userId, userEmail, isMaster }) {
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {/* 마스터 및 본인 삭제 */}
        {(isMaster || (r.userId && (r.userId === userEmail || r.userId === userId))) ? (
    ...
    <>
      {viewer !== null && (
        <PhotoViewer
          ...
          userId={user?.id} userEmail={user?.email} isMaster={isMaster}
        />
      )}
                    ...
                    <div style={{ maxHeight: isOpen ? '420px' : '0px', overflow: 'hidden', transition: 'max-height 0.32s cubic-bezier(0.4,0,0.2,1)' }}>
                      <div style={{ padding: '0 14px 16px', borderTop: T.divider, background: T.expandBg }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px', marginTop: '11px' }}>
                          ...
                          {(isMaster || (user && (r.userId === user.email || r.userId === user.id))) && (
