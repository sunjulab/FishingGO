...
const TIER_BADGE = {
  ...
  BUSINESS_LITE: { bg: 'linear-gradient(135deg,#C0C0C0,#A0A0A0)', color: T.text, label: 'LITE' },
  FREE:          { bg: '#F2F2F7',                                  color: T.textSub, label: 'FREE' },
};
...
export default function UserProfile() {
  ...
  if (loading) return (
    <div style={{ minHeight: '100dvh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '14px', background: T.cardSub }}>
      ...
      <p style={{ fontSize: `calc(14px * var(--fs, 1))`, color: T.textSub, fontWeight: '700' }}>프로필 불러오는 중...</p>
    </div>
  );

  if (notFound) return (
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 20px', background: T.cardSub, textAlign: 'center' }}>
      <div style={{ fontSize: `calc(52px * var(--fs, 1))`, marginBottom: '12px' }}>👤</div>
      <p style={{ fontSize: `calc(18px * var(--fs, 1))`, fontWeight: '900', color: T.text, marginBottom: '8px' }}>사용자를 찾을 수 없습니다</p>
      <p style={{ fontSize: `calc(13px * var(--fs, 1))`, color: T.textSub, marginBottom: '24px' }}>탈퇴했거나 존재하지 않는 닉네임입니다.</p>
  ...
  return (
    <div style={{ minHeight: '100dvh', background: T.cardSub, fontFamily: "'Pretendard', 'Apple SD Gothic Neo', sans-serif" }}>
      {/* 헤더 — ✅ SAFE-AREA: 상단 상태바 자동 회피 */}
      <div style={{ position: 'sticky', top: 0, zIndex: 100, background: T.card, borderBottom: '1px solid #F0F2F7', padding: 'calc(env(safe-area-inset-top, 0px) + 12px) 16px 12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
        <button onClick={() => window.history.length <= 1 ? navigate('/community', { replace: true }) : navigate(-1)} style={{ border: 'none', background: T.cardSub, padding: '8px', borderRadius: '10px', cursor: 'pointer', display: 'flex' }}>
          <ChevronLeft size={20} color="#1A1A2E" />
        </button>
        <span style={{ flex: 1, fontSize: `calc(16px * var(--fs, 1))`, fontWeight: '950', color: T.text, textAlign: 'center' }}>프로필</span>
        <div style={{ width: '36px' }} />
      </div>

      <div style={{ padding: '20px 20px calc(env(safe-area-inset-bottom, 0px) + 24px)' }}>
        {/* 프로필 카드 */}
        <div style={{ background: T.card, borderRadius: '28px', padding: '28px 24px', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', marginBottom: '16px' }}>
          {/* 아바타 + 정보 */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '18px', marginBottom: '22px' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px', flexWrap: 'wrap' }}>
                <span style={{ fontSize: `calc(20px * var(--fs, 1))`, fontWeight: '950', color: T.text }}>{displayName}</span>
          ...
          {/* 통계 4칸 */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1px', background: T.cardSub, borderRadius: '16px', overflow: 'hidden', marginBottom: '18px' }}>
            {[
              ...
              <div key={s.label} style={{ background: T.card, padding: '12px 6px', textAlign: 'center' }}>
                <s.icon size={12} color="#0056D2" style={{ marginBottom: '4px' }} />
                <div style={{ fontSize: `calc(15px * var(--fs, 1))`, fontWeight: '950', color: T.text }}>{s.val}</div>
                <div style={{ fontSize: `calc(9px * var(--fs, 1))`, color: T.textSub, fontWeight: '700' }}>{s.label}</div>
              </div>
            ))}
          </div>
          ...
          {isSelf && (
            <button
              ...
              style={{ width: '100%', padding: '14px', border: '2px solid #0056D2', borderRadius: '16px', fontWeight: '900', fontSize: `calc(15px * var(--fs, 1))`, cursor: 'pointer', background: T.card, color: '#0056D2', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
            >
              내 프로필 관리
            </button>
          )}
        ...
        {/* 가입일 */}
        {profile?.joinedAt && (
          <div style={{ textAlign: 'center', fontSize: `calc(12px * var(--fs, 1))`, color: T.textLight, fontWeight: '600', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }}>
            <Calendar size={12} />
