...
export default function MapHome() {
  ...
  const _enterPoint = useCallback(async (point, fromDashboard = false) => {
    ...
    // ✅ LAST-POINT: 포인트 선택 시 point 객체를 통합 localStorage에 저장
    try { localStorage.setItem('fishinggo_last_point', JSON.stringify(point)); } catch { /* 무시 */ }
  ...
  const DEFAULT_POINT = useMemo(() => {
    // [1순위] 마지막으로 선택했던 포인트 (통합 localStorage 복원)
    try {
