...
const RoleBadge = ({ role }) => {
  if (role === 'owner')   return <span style={{ fontSize: `calc(14px * var(--fs, 1))` }} title="크루장">👑</span>;   // 빨간 왕관
  if (role === 'officer') return <span style={{ fontSize: `calc(14px * var(--fs, 1))` }} title="간부">🫅</span>;      // 핑크 왕관
  return null;
};
        ...
        {messages.map((msg, idx) => {
          ...
          return (
            <div key={String(msg._id || `msg-${idx}`)} style={{ alignSelf: isMe ? 'flex-end' : 'flex-start', maxWidth: '78%' }}>
              ...
              <div style={{
                padding: '10px 14px', borderRadius: '18px', fontSize: `calc(14px * var(--fs, 1))`, lineHeight: '1.5',
                ...
                return (
                  <div key={m.email || i} style={{
                    ...
                    <div style={{
                      ...
                      color: '#fff', fontWeight: '900', fontSize: `calc(16px * var(--fs, 1))`, position: 'relative'
                    }}>
                      {m.name?.[0] || '?'}
                    </div>
                    ...
                    {isOwner && !isThisOwner && !isMe && (
                      <div style={{ display: 'flex', gap: '4px', flexShrink: 0, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                        ...
                        <button
                          ...
                          style={{ border: 'none', background: 'rgba(255,215,0,0.15)', color: '#B8860B', padding: '6px 8px', borderRadius: '10px', fontSize: `calc(11px * var(--fs, 1))`, fontWeight: '800', cursor: 'pointer', whiteSpace: 'nowrap' }}
                        >👑 위임</button>
                        {/* 간부 설정/해제 */}
                        {isThisOfficer ? (
                          <button
                            onClick={() => handleSetRole(m.email, m.name, 'member')}
                            style={{ border: 'none', background: 'rgba(255,45,139,0.1)', color: '#FF2D8B', padding: '6px 10px', borderRadius: '10px', fontSize: `calc(11px * var(--fs, 1))`, fontWeight: '800', cursor: 'pointer', whiteSpace: 'nowrap' }}
                          >
                            간부 해제
                          </button>
                        ) : (
                          <button
                            ...
                            style={{ border: 'none', background: 'rgba(255,45,139,0.08)', color: '#FF2D8B', padding: '6px 10px', borderRadius: '10px', fontSize: `calc(11px * var(--fs, 1))`, fontWeight: '800', cursor: 'pointer', whiteSpace: 'nowrap' }}
                          >
                            🫅 간부
                          </button>
                        )}
                        ...
                        <button
                          ...
                          style={{ border: 'none', background: 'rgba(255,59,48,0.1)', color: '#FF3B30', padding: '6px 10px', borderRadius: '10px', fontSize: `calc(11px * var(--fs, 1))`, fontWeight: '800', cursor: 'pointer' }}
                        >
                          강퇴
                        </button>
                      </div>
