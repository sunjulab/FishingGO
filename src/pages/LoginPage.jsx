...
export default function LoginPage() {
  ...
  const loginWithGoogle = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        setLoading(true);
        const res = await apiClient.post('/api/auth/google', { access_token: tokenResponse.access_token });
        onLoginSuccess(res.data);
      } catch (err) {
        addToast(err.response?.data?.error || '구글 로그인 실패', 'error');
      } finally { setLoading(false); }
    }
  });
    ...
    if (window.Kakao && window.Kakao.isInitialized()) {
      window.Kakao.Auth.login({
        success: async (authObj) => {
          try {
            setLoading(true);
            const res = await apiClient.post('/api/auth/kakao', { access_token: authObj.access_token });
            onLoginSuccess(res.data);
          } catch (err) {
            addToast(err.response?.data?.error || '카카오 로그인 실패', 'error');
          } finally { setLoading(false); }
        },
        fail: (err) => addToast('카카오 로그인 에러', 'error')
      });
    } else {
      addToast('카카오 SDK 초기화 중입니다. 잠시 후 다시 시도해주세요.', 'error');
    }
  };
  ...
  const loginWithNaver = () => {
    ...
    if (!clientId) {
       addToast('네이버 클라이언트 ID가 설정되지 않았습니다.', 'error');
       return;
    }
    const redirectUri = encodeURIComponent(window.location.origin + '/login');
    window.location.href = `https://nid.naver.com/oauth2.0/authorize?response_type=token&client_id=${clientId}&redirect_uri=${redirectUri}&state=NAVER_LOGIN`;
  };
            ...
            {isLogin && (
              <div style={{ marginTop: '20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
                  ...
                  <div style={{ fontSize: `calc(12px * var(--fs, 1))`, color: '#9ca3af', fontWeight: '700' }}>SNS 간편 로그인</div>
                  <div style={{ flex: 1, height: '1px', background: '#e5e7eb' }}></div>
                </div>
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button onClick={() => loginWithGoogle()} style={{
                    flex: 1, height: '50px', borderRadius: '12px', border: '1px solid #e5e7eb',
                  ...
                  <button onClick={loginWithKakao} style={{
                    ...
                    background: '#FEE500', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer'
                  }}>
                    <svg width="26" height="26" viewBox="0 0 24 24"><path fill="#000" d="M12 3c-5.5 0-10 3.5-10 7.8 0 2.8 1.8 5.3 4.5 6.7-.1.5-.5 1.8-.5 1.8s-.1.4.1.5c.2.1.5-.1.5-.1 0 0 1.9-1.3 2.2-1.5.8.2 1.6.3 2.5.3 5.5 0 10-3.5 10-7.8S17.5 3 12 3z"/></svg>
                  </button>
                  <button onClick={loginWithNaver} style={{
                    flex: 1, height: '50px', borderRadius: '12px', border: 'none',
                    background: '#03C75A', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer'
                  }}>
                    <svg width="22" height="22" viewBox="0 0 24 24"><path fill="#fff" d="M16 13l-4-5H8v8h4v-5l4 5h4V8h-4v5z"/></svg>
                  </button>
                </div>
              </div>
            )}
