/**
 * MyPage.jsx 크루 탭 패치 스크립트 (CommonJS)
 * 실행: node patch-mypage-crew.cjs
 */
const fs = require('fs');
...
// 탐색 패턴: records 탭 끝 -> posts 탭 시작 부분 (CRLF/LF 모두 지원)
const patterns = [
...
for (const pattern of patterns) {
  if (content.includes(pattern)) {
    ...
    patched = true;
    const lineType = patterns.indexOf(pattern) === 0 ? 'CRLF' : 'LF';
    console.log('✅ 크루 탭 삽입 성공! (줄바꿈: ' + lineType + ')');
    break;
  }
}

if (!patched) {
  // 디버그: 실제 파일에서 651줄 근처 내용 출력
  console.log('❌ 패턴을 찾지 못했습니다. 파일 내용 확인:');
  const lines = content.split('\n');
  for (let i = 648; i <= 654; i++) {
...
console.log('✅ 브라우저에서 마이페이지 > 내 크루 탭을 확인하세요.');
