...
function applyCoastalTransform(sid, wh, ws, wdDeg) {
  ...
  const maxWave = parseFloat((wh * 1.8 + 0.2).toFixed(1));
...
async function updateAllStationsCache() {
  ...
  const results = await Promise.allSettled(ALL_STATIONS.map(async (sid) => {
    ...
    const finalWave = marine?.wave?.coastal ?? Math.max(0.1, (profile.wave * 1.8 + 0.2) + (lcg(3) * 0.3 - 0.15));
...
app.get('/api/weather/precision', checkSubscriptionValid, async (req, res) => {
  ...
  const fbData = {
    ...
    wave: { coastal: parseFloat(((profile.wave || 0.6) * 1.8 + 0.2).toFixed(1)) },
    layers: {
      upper: mockSst,
