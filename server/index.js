...
app.post('/api/payment/google-iap/verify', verifyToken, async (req, res) => {
  try {
    ...
    const IAP_PLAN_MAP = {
      'kr.fishinggo.app.lite_monthly': { planId: 'BASIC', tier: 'BUSINESS_LITE', amount: 3300,   label: '베이직', days: 31  }, // ✅ Google Play Console KRW 3,300 동기화
...
function calcFishingScoreForStation(sid) {
  ...
  if      (wave > 2.0) score -= 70;
  else if (wave > 1.5) score -= 40;
  else if (wave > 1.0) score -= 15;
  else if (wave >= 0.6) score += 5;
  else if (wave >= 0.3) score += 0;
  else                  score -= 5;
...
function _mapToAliKeyword(category) {
  const map = {
    ...
    '돔':          'sea bream fishing hook rig',
    '농어':        'sea bass minnow lure fishing',
    '광어':        'flounder jig fishing lure',
    '우럭':        'rockfish jigging lure hook',
    '볼락':        'rockfish light game lure soft',
    '학공치':      'halfbeak fishing float rig hook',
    '삼치':        'Spanish mackerel trolling lure jig',
    '갈치':        'cutlassfish belt fishing lure jig',
    '고등어':      'mackerel sabiki rig fishing hook',
    '전갱이':      'horse mackerel sabiki rig lure',
    '방어':        'yellowtail amberjack jigging lure',
    '부시리':      'amberjack heavy jigging lure',
    '참치':        'tuna trolling lure big game',
    '연어':        'salmon fishing lure spoon spinner',
    '송어':        'trout fishing lure spoon soft bait',
    '배스':        'bass fishing soft bait lure jig',
    '잉어':        'carp fishing tackle hair rig boilie',
    '붕어':        'crucian carp fishing float rod',
    '메기':        'catfish fishing bottom rig hook',
    '뱀장어':      'eel fishing bottom rig hook night',
    '오징어':      'squid jigging egi jig lure',
    '한치':        'bigfin squid egi jig fishing',
    '문어':        'octopus fishing jig egi lure',
    '주꾸미':      'webfoot octopus egi jig lure',
    '낙지':        'octopus fishing egi jig lure',
    '망둥이':      'goby fishing hook bottom rig',
    '숭어':        'mullet fishing float rig hook',
    '꽁치':        'Pacific saury sabiki fishing hook',

    // ── 릴/낚싯대 (15종) ────────────────────────────────────────────────────
    '릴':          'spinning fishing reel',
    '스피닝':      'spinning fishing reel 2000 3000 5000',
    '베이트':      'baitcasting fishing reel low profile',
    '전동릴':      'electric fishing reel sea boat',
    '낚시릴':      'spinning fishing reel 2000 3000',
    '낚싯대':      'fishing rod telescopic carbon',
    '루어대':      'lure fishing rod spinning ultralight',
    '원투대':      'surf casting fishing rod long distance',
    '갯바위대':    'rock shore fishing rod spinning',
    '찌낚시대':    'float fishing rod telescopic carp',
    '선상대':      'boat fishing rod jigging heavy',
    '민물대':      'freshwater telescopic fishing pole carp',
    '빙어대':      'ice fishing rod ultralight tip',
    '카본대':      'carbon fiber fishing rod telescopic',
    '글라스대':    'fiberglass fishing rod spinning',

    // ── 루어/채비 (20종) ────────────────────────────────────────────────────
    '루어':        'fishing lure set soft bait',
    '에기':        'squid egi jig lure',
    '에깅':        'squid egi jig spinning lure',
    '지그':        'fishing metal jig lure',
    '지그헤드':    'jig head fishing hook weight set',
    '웜':          'soft plastic worm fishing lure',
    '소프트웜':    'soft fishing worm lure bass',
    '미노우':      'minnow fishing lure hard bait',
    '바이브':      'vibration fishing lure vibe lure',
    '탑워터':      'topwater popper fishing lure surface',
    '스푼':        'fishing spoon lure metal spinner',
    '스피너':      'spinner fishing lure inline blade',
    '크랭크베이트': 'crankbait fishing lure diving wobbler',
    '러버지그':    'rubber jig fishing lure bass',
    '섀드':        'shad soft plastic fishing lure',
    '그럽':        'grub soft plastic fishing lure',
    '훅':          'fishing hook set assorted sizes',
    '낚시바늘':    'fishing hook set size assorted',
    '봉돌':        'fishing sinker weight lead set',
    '찌':          'fishing float bobber set adjustable',

    // ── 낚시방법 (15종) ─────────────────────────────────────────────────────
    '갯바위':      'rock shore fishing tackle rig',
    '선상':        'boat jigging fishing tackle',
    '민물':        'freshwater fishing tackle carp',
    '내만':        'inshore fishing lure tackle set',
    '방파제':      'pier breakwater fishing tackle rig',
    '원투':        'surf casting fishing sinker rig',
    '찌낚시':      'float fishing bobber rig tackle',
    '루어낚시':    'lure fishing soft bait jig set',
    '지깅':        'jigging lure metal jig fishing',
    '타이라바':    'tai rubber jig snapper fishing lure',
    '에깅낚시':    'squid egi jig spinning tackle',
    '빙어낚시':    'ice fishing rod lure tip ultralight',
    '배스낚시':    'bass fishing soft bait jig lure',
    '트롤링':      'trolling fishing lure big game deep',
    '플라이낚시':  'fly fishing lure dry fly wet fly',

    // ── 장비/소품 (20종) ────────────────────────────────────────────────────
    '낚시줄':      'fishing line PE braid',
    '합사줄':      'PE braided fishing line 4 strand',
    '카본줄':      'fluorocarbon fishing leader line',
    '목줄':        'fluorocarbon fishing leader line',
    '릴대':        'fishing rod reel combo spinning set',
    '낚시가방':    'fishing bag backpack tackle storage',
    '낚시박스':    'fishing tackle box storage organizer',
    '낚시의자':    'fishing chair folding portable seat',
    '뜰채':        'fishing landing net folding handle',
    '쿨러':        'fishing cooler bag ice fish keep',
    '집어등':      'fishing light LED underwater green',
    '낚시장갑':    'fishing gloves anti-slip waterproof',
    '편광선글라스': 'polarized fishing sunglasses UV protection',
    '낚시모자':    'fishing hat sun protection cap UV',
    '낚시복':      'fishing shirt pants UPF UV protection',
    '낚시화':      'fishing wading boots waterproof non-slip',
    '낚시조끼':    'fishing vest jacket multi pocket',
    '전동채비기':  'automatic fishing rig hook tyer machine',
    '낚시타이머':  'fishing timer alarm bite alarm',
    '입질알람':    'fishing bite alarm electronic sensor',

    // ── 장소/시즌 ──────────────────────────────────────────────────────────
    '스피닝릴':    'spinning fishing reel 2000 3000 5000',
    '베이트릴':    'baitcasting fishing reel low profile',
    '전동릴':      'electric fishing reel boat sea',
    '루어낚시대':  'lure fishing rod spinning ultralight',
    '원투낚시대':  'surf casting fishing rod long distance beach',
    '갯바위대':    'rock shore fishing rod medium heavy',
    '민물낚시':    'freshwater carp fishing tackle float',
